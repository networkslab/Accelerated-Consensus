...
out_folder = 'fig3/cons_CIRC_Lin_poly_paper_';
FontSize_lab = 24;
FontSize_ax = 18;
MarkerSize = 8;

DFvec = [10; 25; 50; 100; 200];
DFbvec = [50; 200; 1000; 500; 1000];
shift_mag_vec = [5; 12; 25; 50; 100];
...
for ii=1:length(Nsim)
    ...
    h(3)=semilogy(downsample((1:Mc)', DF), ...
        downsample(sqrt(MSE_mhM3), DF), '-dr');
...
h=0;
hold off;
h(1)=semilogy(Nsim, Tave_mh, '-b^');
hold on;
h(2)=semilogy(Nsim, Tave_opt, '-+k');
h(3)=semilogy(Nsim, Tave_mhM3, '-dr');
h(4)=semilogy(Nsim, Tave_mh_pwrM3, '-.xg');
h(5)=semilogy(Nsim, Tave_optM3, '-sk');
...
ylim([min(1e2); max(2e5)]); % ylim([min(Tave_optM3); max(Tave_mh)]);
...
h=0;
hold off;
h(1)=loglog(Nsim, Tave_mh./Tave_mhM3, '-dr');
hold on;
h(2)=loglog(Nsim, Tave_mh./Tave_mh_pwrM3, '-.xg');
h(3)=loglog(Nsim, Tave_mh./Tave_optM3, '-sk');
h(4)=loglog(Nsim, Tave_mh./Tave_opt, '-+k');
