...
% where topology is modelled as a Path Graph and the
% initialization is "Slope". Plot all results.
...
Nsim=[25; 50; 100; 150; 200];
% Nsim = 200;
in_folder = 'sim_results_2/cons_CIRC_Lin_POLY_N';
out_folder = 'fig3/cons_CIRC_Lin_poly_acc_paper_';
FontSize_lab = 24;
FontSize_ax = 18;
MarkerSize = 8;
DF = 10;
...
DFvec = [30; 50; 100; 150; 200];
DFbvec = [50; 200; 1000; 500; 1000];
shift_mag_vec = DFvec/2;
...
for ii=1:length(Nsim)
    ...
    x_ub = 3*sum(10*log10(MSE_mhM3) >= mse_lb);
    ...
    legend('Opt', 'MH-PolyFilt3', 'MH-Proposed', 'MH-Proposed-Est', 'MH-PolyFilt7', 'location', 'SouthEast');
...
h=0;
hold off;
h(1)=semilogy(Nsim, Tave_mh_polyO3, '-vc');
hold on;
h(2)=semilogy(Nsim, Tave_opt, '-k+');
h(3)=semilogy(Nsim, Tave_mh_polyO20, '-b>');
h(4)=semilogy(Nsim, Tave_mhM3, '-rd');
...
ylim([min(1e2); max(1e5)]); % ylim([min(Tave_mhM3); max(Tave_opt)]);
...
h=0;
hold off;
h(1)=loglog(Nsim, Tave_mh ./ Tave_mhM3, '-rd');
hold on;
h(2)=loglog(Nsim, Tave_mh ./ Tave_mh_polyO20, '-b>');
h(3)=loglog(Nsim, Tave_mh ./ Tave_opt, '-+k');
h(4)=loglog(Nsim, Tave_mh ./ Tave_mh_polyO3, '-vc');
...
xlim([min(Nsim); max(Nsim)]);
xlabel('{\it N}', 'FontSize', FontSize_lab);
ylabel('T_{ave}(W) / T_{ave}(\Phi)', 'FontSize', FontSize_lab);
set(gca,'XTick', [25; 50; 100; 150; 200]);
