function [l2_est, l2_M3, MSE, x_it] = ...
    do_consensus_acc(M, W, E, l2, theta, x, EstMethod, Nit, NitM)
% Estimate the eigenvalue of the foundational weight matrix (if instructed)
% calculate the optimal value of mixing parameter,
% perform accelerated consensus iterations,
...
elseif (strcmp(EstMethod,'PWRN'))
    % distributed eigenvalue estimation based on power iterations
    l2_est = estimate_l2_new(W, E, N, NitM);
...
for i = 2:M
    if (i > 2)
        x_it(:, i) = WM3 * x_it(:, i-1);
        MSE(i) = one_vec * (x_it(1:N, i) - x_mean).^2 ;
    else
        x_it(N+1:2*N, i) = x_it(1:N, i-1);
        x_it(1:N, i) = W * x_it(1:N, i-1);
        MSE(i) = one_vec * (x_it(1:N, i) - x_mean).^2 ;
    end;
end;
